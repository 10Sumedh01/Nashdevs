...
class Companion:
    def __init__(self, pos, comp_type):
        ...
        self.image = pygame.transform.scale(self.image, (self.size, self.size))  # Scale the image to match the size
    ...
    def update(self, player, zombies, obstacles):
        # Follow the main player with a lag
        ...
        if direction.length() > 0:
            ...
            collision = any(new_rect.colliderect(obs.get_rect()) for obs in obstacles)
            if not collision:
                self.pos = new_pos
